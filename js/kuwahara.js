(function(imageproc) {
    ...
    imageproc.kuwahara = function(inputData, outputData, size) {
        ...
        function regionStat(x, y) {
            ...
            var meanValue = 0;
            var regionSize = Math.floor(size/2) + 1;
            var halfRegionSize = Math.floor(regionSize/2);
            for (var j = -halfRegionSize; j <= halfRegionSize; j++) {
                for (var i = -halfRegionSize; i <= halfRegionSize; i++) {
            ...
            var divisor = regionSize * regionSize;
            meanR /= divisor;
            meanG /= divisor;
            meanB /= divisor;
            meanValue /= divisor;
        ...
        var distanceToSubRegion = Math.floor(size/4);
        for (var y = 0; y < inputData.height; y++) {
            for (var x = 0; x < inputData.width; x++) {
                // Find the statistics of the four sub-regions
                var regionA = regionStat(x - distanceToSubRegion, y - distanceToSubRegion, inputData);
                var regionB = regionStat(x + distanceToSubRegion, y - distanceToSubRegion, inputData);
                var regionC = regionStat(x - distanceToSubRegion, y + distanceToSubRegion, inputData);
                var regionD = regionStat(x + distanceToSubRegion, y + distanceToSubRegion, inputData);
